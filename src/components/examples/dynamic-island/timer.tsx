...
function TimerButton({ isPaused, onToggle }: TimerButtonProps) {
  return (
    <motion.button
...
function useTimer(paused: boolean) {
  ...
  // Ensure positive blur values by using Math.max
...
function Counter({ paused }: { paused?: boolean }) {
  const digits = useTimer(paused ?? false);
...
// Move these to separate files in practice
const PlayIcon = () => (
  <motion.svg
    key="play"
